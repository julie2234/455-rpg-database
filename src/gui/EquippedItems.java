...
public class EquippedItems extends JPanel {
	private String[] attributes = {"characterID", "boots", "hat", "gloves", "breastplate", "mainHand", "offHand", "legs"};
	
	public EquippedItems() throws Exception {
		super();
		initialize();
	}
			...
			public void actionPerformed(final ActionEvent the_event) {
				...
				if (confirm == JOptionPane.YES_OPTION) {
					StringBuilder query = new StringBuilder(
							"DELETE FROM `Equipped Items` where ");
			...
			public void actionPerformed(final ActionEvent the_event) {
				...
				if ((searchBy.equals("characterID") || searchBy.equals("characterLevel")
					...
					if (query.substring(query.length() - 2, query.length()).equals(
						...
						query.append(" FROM `Equipped Items` where " + searchBy
								+ " = \"" + condition + "\";");
						JTable table = null;
						try {
		...
		viewAllButton.addActionListener(new ActionListener() {
			public void actionPerformed(final ActionEvent the_event) {
				...
				if (query.substring(query.length() - 2, query.length()).equals(
						", ")) {
					query.delete(query.length() - 2, query.length());
					query.append(" FROM `Equipped Items`;");
					JTable table = null;
					try {
						table = Database.executeQuery(query.toString());
					} catch (Exception e) {
